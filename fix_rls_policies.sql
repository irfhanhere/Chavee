...
-- 1. Enable RLS on conversation_participants (if not already enabled)
ALTER TABLE public.conversation_participants ENABLE ROW LEVEL SECURITY;

-- 2. Drop the existing recursive SELECT policies on conversation_participants
...
-- 3. Create a SECURITY DEFINER helper function to check membership.
...
-- 4. Create the new non-recursive SELECT policy using the helper function
CREATE POLICY "Users can read conversation participants they share a conversation with" ON public.conversation_participants
FOR SELECT
TO authenticated
USING (
...
-- 5. Create/Update INSERT policy to allow adding conversation participants
...
CREATE POLICY "Allow authenticated users to insert participants" ON public.conversation_participants
FOR INSERT
TO authenticated
WITH CHECK (
